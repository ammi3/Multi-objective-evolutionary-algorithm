import random, copy, math, turtle, os, xlwt, xlrd
...
class MOEAD:
    ...
    def run(self):
        ...
        while (t <= self.maxGen):
            ...
            temp_IGDValue = self.getIGDValue(self.PF_ref, PF_know)
            if self.IGDValue == None:
                self.IGDValue = temp_IGDValue
            else:
                if temp_IGDValue < self.IGDValue:
                    self.IGDValue = temp_IGDValue
            print('Generation ', t, ': ', self.IGDValue)
        ...
        return EP_list, self.IGD_list  # 返回最终的非支配解集
...
class SMD:
    def __init__(self):
        ...
        self.g_i_j = None       # The channel gain between the SMD and SeNB Si
